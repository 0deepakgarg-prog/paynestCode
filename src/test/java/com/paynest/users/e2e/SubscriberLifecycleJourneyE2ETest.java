...
class SubscriberLifecycleJourneyE2ETest {

    private static final String TENANT_ID = "tenant-1";
    private static final String TENANT_SCHEMA = "public";
    private static final String SUBSCRIBER_ACCOUNT_ID = "AC202603200001";
    private static final String SUBSCRIBER_MOBILE = "9876543210";
    ...
    void setUp() {
        ...
        when(userDetailsService.loadUserByUsername(SUBSCRIBER_ACCOUNT_ID)).thenReturn(
                new User(SUBSCRIBER_ACCOUNT_ID, "N/A", AuthorityUtils.NO_AUTHORITIES)
        );
    ...
    @Test
    void subscriberJourney_shouldRegisterChangeDefaultPinPayAndDeleteSubscriber() {
        Account registeredSubscriber = new Account();
        registeredSubscriber.setAccountId(SUBSCRIBER_ACCOUNT_ID);

        doNothing().when(accountService).generateOtpForRegistration(any(RegistrationRequest.class));
        when(accountService.registerUser(any(RegistrationRequestWithOtp.class))).thenReturn(registeredSubscriber);
        ...
        doNothing().when(accountService).deleteSubscriber(SUBSCRIBER_ACCOUNT_ID);
        ...
        given()
                ...
                .statusCode(200)
                .body("status", equalTo("SUCCESS"))
                .body("accountId", equalTo(SUBSCRIBER_ACCOUNT_ID));
        ...
        String subscriberAccessToken =
                given()
                        .contentType(ContentType.JSON)
                        .header("X-Tenant-Id", TENANT_ID)
                        .body("""
                                {
                                  "requestId": "req-login-subscriber-001",
                                  "user": {
                                    "identifierType": "MOBILE",
                                    "identifierValue": "%s"
                                  },
                        ...
                        .body("status", equalTo("SUCCESS"))
                        .body("accountId", equalTo(SUBSCRIBER_ACCOUNT_ID))
                        .body("accessToken", equalTo(SUBSCRIBER_TOKEN))
                        .extract()
                        .path("accessToken");
        ...
        given()
                .contentType(ContentType.JSON)
                .header("X-Tenant-Id", TENANT_ID)
                .header("Authorization", "Bearer " + subscriberAccessToken)
                .body("""
                        {
                          "operationType": "U2U_TRANSFER",
                          "initiatedBy": "DEBITOR",
                          "paymentReference": "pay-ref-001",
                          "comments": "subscriber initiated transfer",
                          "debitor": {
        ...
        given()
                ...
                .when()
                .delete("/api/v1/account/subscriber/{accountId}", SUBSCRIBER_ACCOUNT_ID)
                .then()
                .statusCode(200)
                .body("status", equalTo("SUCCESS"))
        ...
        inOrder.verify(authService).login(any(AuthLoginRequest.class));
        inOrder.verify(accountService).deleteSubscriber(SUBSCRIBER_ACCOUNT_ID);
    }
